...
namespace EbonianMod.Content.Items.Weapons.Melee
{
    public class BaseballBat : ModItem
    {
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            {
                velocity = new Vector2(player.velocity.X, player.velocity.Y - Item.shootSpeed);
            }
                ...
                else if (Projectile.ai[2] > 0.5f && Projectile.ai[1] != 100)
                {
                    foreach (Projectile projectile in Main.ActiveProjectiles)
                    {
                        if (projectile.type == ProjectileType<Baseball>() && Vector2.Distance(Projectile.Center, projectile.Center) < 50)
                        {
                            ...
                            {
                                projectile.velocity = (Main.MouseWorld - projectile.Center).SafeNormalize(Vector2.UnitY) * Clamp(MathF.Pow(projectile.velocity.Length(), 2) / 9, 14, 34);
                                if (Distance(Main.MouseWorld.X, player.Center.X) < 8)
