...
{
    public override string Texture => Helper.AssetPath + "Items/Weapons/Melee/GhizasWheel";
...
{
    public override string Texture => Helper.AssetPath + "Items/Weapons/Melee/GhizasWheelP";
    float holdOffset = 30;
    public override void SetDefaults()
    {
    ...
    public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            if (Main.rand.NextBool())
                Dust.NewDustPerfect(pos + Helper.FromAToB(pos, target.Center).RotatedByRandom(MathHelper.PiOver4 * 0.7f) * 16, DustType<LineDustFollowPoint>(), Helper.FromAToB(pos, target.Center).RotatedByRandom(MathHelper.PiOver2) * Main.rand.NextFloat(5, 8), 0, new Color(147, 255, 36, 255), Main.rand.NextFloat(0.05f, 0.1f));
            else
                Dust.NewDustPerfect(pos + Helper.FromAToB(pos, target.Center).RotatedByRandom(MathHelper.PiOver4 * 0.7f) * 16, DustType<SparkleDust>(), Helper.FromAToB(pos, target.Center).RotatedByRandom(MathHelper.PiOver2) * Main.rand.NextFloat(5, 8), 0, new Color(147, 255, 36, 255), Main.rand.NextFloat(0.025f, 0.075f));
        }
    }
    ...
    public override void OnSpawn(IEntitySource source)
    {
        ...
        slot = SoundEngine.PlaySound(Sounds.ghizasWheel);
        if (SoundEngine.TryGetActiveSound(slot, out var sound))
        {
            sound.Volume = 0;
        }
        slot2 = SoundEngine.PlaySound(Sounds.ghizasWheel.WithPitchOffset(0.35f));
    ...
    {
        Texture2D wheel = Helper.GetTexture(Texture + "_Wheel").Value;
        Vector2 pos = Projectile.Center + new Vector2(22, -16).RotatedBy(Projectile.rotation);
