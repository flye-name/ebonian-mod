...
{
    public override string Texture => Helper.AssetPath + "Items/Weapons/Melee/Equilibrium";
...
{
    public override string Texture => Helper.AssetPath + "Items/Weapons/Melee/EquilibriumP";
...
: (2 - MathF.Pow(2, -20 * x + 10)) / 2;
    ...
    public override void OnHit(NPC target, NPC.HitInfo hit, int damageDone)
    {
        ...
        if (!_hit)
        {
            SoundEngine.PlaySound(Sounds.FleshImpact, Projectile.Center);
            Vector2 dir = Vector2.Normalize(Main.MouseWorld - player.Center);
    ...
    public override void ExtraAI()
    {
        ...
        {
            SoundEngine.PlaySound(Sounds.HeavySwing, Projectile.Center);
        }
        ...
        Arm(player);
        if (swingProgress.InRange(0.5f, 0.45f) && swingProgress < 0.85f && Projectile.timeLeft % 2 == 0)
            oldP.Add(rot);
    }
...
: (2 - MathF.Pow(2, -20 * x + 10)) / 2;
    ...
    public override void ExtraAI()
    {
        ...
        float angle = MathHelper.Lerp(-MathHelper.PiOver2 - MathHelper.PiOver4 * 0.5f, MathHelper.Pi + MathHelper.PiOver4 * 0.5f, -swingProgress + 1);
        float rot = angle + (Projectile.ai[1] == 1 ? 0 : MathHelper.Pi) + MathHelper.PiOver2;
        if (swingProgress.InRange(0.5f, 0.45f) && swingProgress < 0.85f)
            oldP.Add(rot);
    }
    List<float> oldP = new List<float>(30);
    public override bool PreDraw(ref Color lightColor)
