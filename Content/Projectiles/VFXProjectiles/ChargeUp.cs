...
public class ChargeUp : ModProjectile
{
    ...
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        float ringScale = MathHelper.Lerp(1, 0, MathHelper.Clamp(Projectile.localAI[0] * 3.5f, 0, 1));
        if (ringScale > 0.01f)
        {
            for (float i = 0; i < max; i++)
            {
                ...
                float scale = rand.NextFloat(0.1f, .5f) * scaleFactor;
                Vector2 offset = new Vector2(rand2.NextFloat(300, 400) * (ringScale + rand2.NextFloat(-0.2f, 0.5f)) * scale, 0).RotatedBy(angle);
                Main.spriteBatch.Draw(tex, Projectile.Center + offset - Main.screenPosition, null, Color.Lerp(Color.DarkRed, Color.Maroon, Projectile.localAI[0] * 2) * ringScale, angle, tex.Size() / 2, new Vector2(MathHelper.Clamp(Projectile.localAI[0] * 6.5f, 0, 1), ringScale) * scale * 0.2f * 4, SpriteEffects.None, 0);
            }
        }
    ...
    public override void AI()
    {
        if (Projectile.timeLeft < 50)
            Projectile.localAI[0] = MathHelper.Lerp(Projectile.localAI[0], 1f, 0.007f);
    ...
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        if (ringScale > 0.01f)
        {
            for (float i = 0; i < max; i++)
            {
                ...
                Vector2 offset = new Vector2(rand2.NextFloat(300, 400) * (ringScale + rand2.NextFloat(-0.2f, 0.5f)) * scale, 0).RotatedBy(angle);
                Main.spriteBatch.Draw(tex, Projectile.Center + offset - Main.screenPosition, null, Color.Lerp(Color.Olive, Color.Green, Projectile.localAI[0] * 2) * ringScale, angle, tex.Size() / 2, new Vector2(MathHelper.Clamp(Projectile.localAI[0] * 6.5f, 0, 1), ringScale) * scale * 0.2f * 4, SpriteEffects.None, 0);
            }
        }
        rand = new UnifiedRandom(seed + 1);
        ringScale = MathHelper.Lerp(1, 0, MathHelper.Clamp(Projectile.ai[1] * 3.5f, 0, 1));
...
public class ArchmageChargeUp : ModProjectile
{
        ...
        XareusRendering.DrawCache.Add(() =>
        {
            ...
            if (ringScale > 0.01f)
            {
                for (float i = 0; i < max; i++)
                {
                    ...
                    Main.spriteBatch.Draw(tex, Projectile.Center + offset - Main.screenPosition, null, Color.White * ringScale, angle, tex.Size() / 2, new Vector2(MathHelper.Clamp(Projectile.localAI[0] * 6.5f, 0, 1), ringScale) * scale * 0.2f * 4, SpriteEffects.None, 0);
                }
            }
            rand = new UnifiedRandom(seed + 1);
            ringScale = MathHelper.Lerp(1, 0, MathHelper.Clamp(Projectile.ai[1] * 3.5f, 0, 1));
    ...
    public override void AI()
    {
        if (Projectile.timeLeft < 60)
            Projectile.localAI[0] = MathHelper.Lerp(Projectile.localAI[0], 1f, 0.006f);
        if (Projectile.timeLeft < 50)
